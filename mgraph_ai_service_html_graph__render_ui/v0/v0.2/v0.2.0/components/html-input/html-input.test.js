...
QUnit.module('HTML Input', function(hooks) {
    ...
    QUnit.skip('emits html-changed event on input', async function(assert) {
        // Skipped: test was not stable due to debounce timing
    ...
    QUnit.test('[bug] typing clears sample selection', async function(assert) {
        const input = await TestUtils.createComponent('html-input');
        const textarea = input.querySelector('#html-input');
        const select = input.querySelector('#sample-select');
        ...
        assert.strictEqual(select.value, '', '[bug] sample selection is cleared on typing');
    });
