...
class Html_Graph__Export__Service(Type_Safe):                                             # Unified export service for Html_MGraph
    ...
    def to_tree(self, request: Schema__Graph__From_Html__Request) -> dict:
        with capture_duration() as duration:
            config      = self.create_config(request)
            html_mgraph = self.html_to_mgraph(request.html)
    ...
    def to_tree_text(self, request: Schema__Graph__From_Html__Request) -> dict:
        with capture_duration() as duration:
            config      = self.create_config(request)
            html_mgraph = self.html_to_mgraph(request.html)
            exporter    = Html_MGraph__To__Tree_View(html_mgraph=html_mgraph, config=config)
